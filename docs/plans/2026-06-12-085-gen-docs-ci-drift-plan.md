...
- Add `"bundle:mcpb"` (D3). **Packer RESOLVED (2026-06-12): `@anthropic-ai/mcpb@2.1.2`** (official MCPB CLI, bin `mcpb`; pin as a devDependency). The script builds `src` → `bundle/server/` (e.g. a `tsc` emit to that out-dir, mirroring how `bundle/server/` is produced today) then packs: `mcpb pack bundle/ <name>.mcpb` (input dir first, output second; the manifest must sit at the packed dir's root, which `bundle/manifest.json` already does). Keep `gen:docs` and `bundle:mcpb` separate (docs drift check must not require packing a zip). **#85 commits this script but does NOT invoke it in per-PR CI** — it is consumed by #84's release workflow (D3).
...
- **Manifest validity**: run the official validator **`npx mcpb validate bundle/`** (from `@anthropic-ai/mcpb`) — it checks against the MCPB spec, so prefer it over hand-rolled JSON assertions. Also assert in-test that the required top-level fields are present (`manifest_version`, `name`, `version`, `description`, `author`, `server`, `tools`) and `tools` is non-empty. `manifest_version: "0.3"` is current (spec last updated 2025-12-02) — **our manifest is already 0.3, no migration needed**. (The generator must therefore also emit `description` + `author` from `package.json`, not just name/version.)
...
- **`.mcpb` packer — RESOLVED (2026-06-12, Q3 closed):** the official packer is **`@anthropic-ai/mcpb@2.1.2`** (bin `mcpb`; the renamed successor to `@anthropic-ai/dxt`). Pin it as a devDependency; build with `mcpb pack bundle/ <out>.mcpb` and validate with `mcpb validate bundle/`. No `zip` fallback needed. (The packer was the one blocking pre-implementation gate for the `bundle:mcpb` script; it is cleared.)
...
1. **`.mcpb` packer — RESOLVED (2026-06-12).** Packer = **`@anthropic-ai/mcpb@2.1.2`** (official MCPB CLI, bin `mcpb`; pin as a devDependency). `bundle:mcpb` = `mcpb pack bundle/ <out>.mcpb`; manifest validity via `mcpb validate bundle/`. `manifest_version: "0.3"` is current (no migration). The blocking pre-implementation gate is **cleared**.
