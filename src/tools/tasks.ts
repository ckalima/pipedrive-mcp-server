  ...
  // U2: Write tools
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        parent_task_id: { type: ["number", "null"], description: "ID of the parent task (null for root-level)" },
        description: { type: ["string", "null"], description: "Task description (null to clear)" },
        ...
        due_date: { type: ["string", "null"], description: "Task due date (YYYY-MM-DD, null to clear)" },
        start_date: { type: ["string", "null"], description: "Task start date (YYYY-MM-DD, null to clear)" },
        assignee_id: { type: ["number", "null"], description: "Assignee user ID (null to unassign)" },
        assignee_ids: {
          type: "array",
          items: { type: "number" },
          description: "Array of assignee user IDs (max 10)",
        },
        priority: { type: ["number", "null"], description: "Task priority (integer >= 0, or null to unset)" },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        parent_task_id: { type: ["number", "null"], description: "ID of the parent task (null to make root-level)" },
        description: { type: ["string", "null"], description: "Task description (null to clear)" },
        is_done: { type: "boolean", description: "Mark as done (true/false)" },
        is_milestone: { type: "boolean", description: "Mark as milestone (true/false); a milestone task must have a due_date" },
        due_date: { type: ["string", "null"], description: "Task due date (YYYY-MM-DD, null to clear)" },
